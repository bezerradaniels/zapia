...
export default function BillingPage() {
  ...
  const downgradeCheck = useDowngradeCheck(
    store?.id,
    currentPlanId,
    pendingPlanId,
  );
  ...
  const handleStartCheckout = (planId: PlanId) => {
    ...
    if (downgradeCheck.needsProductSelection && planId === pendingPlanId)
      return;
    ...
    <>
      {showDowngradeModal && pendingPlanId && (
        <DowngradeProductSelector
          ...
          newLimit={
            ({ basico: 10, pro: 100, premium: Infinity } as const)[
              pendingPlanId
            ] as number
          }
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="text-lg font-semibold">
              ...
              <span className="text-[#0bfeda]">
                {planList.find((p) => p.id === currentPlanId)
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {billingPeriod === "annual" && (
                        <span className="rounded-full bg-slate-900 px-2 py-0.5 text-[11px] font-bold text-[#0bfeda]">
                          -
                          {Math.round(
                            ((PLANS[plan.id].priceInCents * 12 -
                              PLANS[plan.id].priceInCentsAnnual) /
                              (PLANS[plan.id].priceInCents * 12)) *
                              100,
                          )}
                          %
                  ...
                  <div>
                    <span className="text-3xl font-bold tracking-tighter">
                      {billingPeriod === "annual"
                        ? formatMoney(
                            Math.round(PLANS[plan.id].priceInCentsAnnual / 12),
                          )
                    ...
                    {billingPeriod === "annual" && (
                      <>
                        ...
                        <p className="mt-1 text-xs font-medium text-black">
                          ...
                          {formatMoney(PLANS[plan.id].priceInCentsAnnual)}
                        </p>
                      </>
                    )}
