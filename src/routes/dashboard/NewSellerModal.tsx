    ...
    <div className="mt-3 flex flex-col gap-3">
      ...
      {search && (
        <ul className="max-h-48 overflow-y-auto rounded-lg border border-z-border bg-white">
          ...
          {filtered.length === 0 && (
            <li className="px-3 py-3 text-center text-xs text-z-text-muted">
...
function NewSellerModalContent({
  onClose,
  storeId,
  storeSlug,
}: Omit<Props, "open" | "storeWhatsapp">) {
  ...
  async function onSubmit(values: FormValues) {
    await createCatalog.mutateAsync({
      ...
      whatsapp_phone: values.use_store_whatsapp
        ? null
        : values.whatsapp_phone || null,
              ...
              <div className="flex flex-1 flex-col gap-4">
                    ...
                    <div>
                      ...
                      <PhoneInput
                        className={cn(
                          ...
                          useStoreWhatsapp
                            ? "border-z-border bg-z-bg2 opacity-60"
                            : "border-z-border",
                        )}
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        <label className="text-xs text-z-text-muted">
                          Usar o mesmo WhatsApp da loja
                        </label>
                        <Controller
                ...
                <section className="rounded-2xl border border-z-border bg-white p-6">
                  ...
                  <p className="mb-4 text-xs text-z-text-muted">
                    Conceder acesso a este vendedor para que ele possa acessar o
                    seu Zapia.
                  </p>
                  <div className="flex items-center justify-between rounded-xl border border-z-border px-4 py-3">
                    <label className="text-sm text-z-text">
                      Habilitar acesso ao Zapia para Vendedores
                    </label>
                    <Controller
                  ...
                  {hasDashboardAccess && (
                    <p className="mt-3 text-xs text-z-text-muted">
                      Após salvar, o vendedor receberá instruções de acesso no
                      e-mail cadastrado (se fornecido).
                    </p>
                  )}
                </section>
              </div>
                ...
                {catalogProducts === "all" && (
                  <div className="mt-4 flex items-start gap-2 rounded-xl bg-amber-50 px-3 py-3">
                    ...
                    <p className="text-xs text-z-text-muted">
                      Todos os produtos serão mostrados no catálogo deste
                      vendedor.
                    </p>
                  </div>
                )}
