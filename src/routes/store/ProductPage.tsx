    ...
    return {
      ...
      values: values.map((value) => {
        const matchingOptions = options.filter(
          (option) =>
            getOptionAttributes(option, fallbackLabel)[label] === value,
        );
        return {
          value,
          isOutOfStock:
...
function findMatchingVariationOption(
  ...
  return (
    options.find((option) => {
      ...
      return groupLabels.every(
        (label) => attributes[label] === selectedValues[label],
      );
    }) ?? null
  );
}
...
export default function ProductPage() {
  ...
  useDocumentMeta({
    title: p
      ? `${toTitleCase(p.name)} - ${store.name}`
      : buildStoreTitle(store),
    description: p
      ? truncate(
  ...
  const related = useMemo<Product[]>(() => {
    ...
    const sameBrand = p.brand
      ? others.filter((x) => x.brand === p.brand && !sameCategory.includes(x))
      : [];
  ...
  const handleVariationSelect = (name: string) => {
    ...
    setQty((current) =>
      option?.stock == null
        ? current
        : Math.min(current, Math.max(option.stock, 1)),
    );
  };
  ...
  const handleVariationValueSelect = (label: string, value: string) => {
    ...
    setQty((current) =>
      match.stock == null
        ? current
        : Math.min(current, Math.max(match.stock, 1)),
    );
  };
  ...
  return (
    <div className="mx-auto w-full max-w-[800px] px-4 py-4 pb-28 sm:px-6 sm:py-6 sm:pb-8">
              ...
              <span className="text-[28px] font-bold leading-none tracking-tight text-z-ink sm:text-[34px]">
                {finalPrice === 0
                  ...
                  : (() => {
                      ...
                      return (
                        <>
                          <span className="text-[17px] sm:text-[20px]">
                            R$&nbsp;
                          </span>
                          {intPart}
                          {decPart && (
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3">
              <div className="flex shrink-0 items-center gap-3 sm:gap-4">
                <span className="text-[14px] font-bold text-z-ink">
                  Quantidade
                </span>
                <QuantityStepper
                  qty={displayedQty}
                  max={stock ?? undefined}
                  onChange={(next) =>
                    cartItem ? updateQty(cartKey, next) : setQty(next)
                  }
                />
              </div>
      ...
      {p.description && (
        <div
          className="mt-6 rounded-2xl border bg-white p-5 shadow-sm sm:p-6"
          style={{ borderColor: "#cbd5e1" }}
        >
          <h2 className="mb-3 text-[15px] font-bold text-z-ink">Descrição</h2>
          <div
            ...
            >
              ...
              {itemQuantity > 0 && (
                <span
                  ...
                  style={
                    isSelected ? { color: "var(--store-primary)" } : undefined
                  }
                >
                  ({itemQuantity})
                </span>
