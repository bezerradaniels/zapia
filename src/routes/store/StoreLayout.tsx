...
export default function StoreLayout() {
  ...
  const showFloatingCart =
    cartCount > 0 && !hideFloatingCartRoutes.has(storeRoute);
  ...
  return (
    <OwnerModeContext.Provider value={{ isOwner, ownerMode }}>
      ...
      >
        ...
        />
        {showHero && (
          <StoreHero store={store} isOwner={isOwner} ownerMode={ownerMode} />
        )}
    ...
    >
      ...
      >
        <div className="relative flex min-w-0 flex-1 items-center gap-2 sm:gap-2.5">
          <Link
            to={homePath}
            className="flex min-w-0 flex-1 items-center gap-2 text-white sm:gap-2.5"
          >
            <div
        ...
        <div className="flex shrink-0 items-center gap-1 sm:gap-2">
          ...
          >
            ...
            {cartCount > 0 && (
              <span className="absolute -right-1 -top-1 flex h-[18px] min-w-[18px] items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold tabular-nums text-white animate-pulse sm:static sm:ml-1.5 sm:h-5 sm:min-w-5 sm:text-[11px]">
                {cartCount}
              </span>
            )}
          </Link>
  ...
  if (store.banner_url) {
    return (
      <div className="mx-auto w-full max-w-[800px] px-4 pt-4 sm:px-6">
        <div
          className="relative w-full overflow-hidden rounded-2xl bg-z-bg2"
          style={{ aspectRatio: "8/3", maxHeight: "300px" }}
        >
          <OptimizedImage
...
const SHIPPING_METHOD_MAP: Record<
  ShippingMethod,
  { icon: IconSvgElement; label: string }
> = {
          ...
          {store.delivery_hours?.length > 0 && (
            <>
              ...
              {store.delivery_hours.map((slot, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 text-[13px] text-z-text-muted"
                >
                  <HugeiconsIcon
                    icon={Clock01Icon}
                    size={15}
                  ...
                  />
                  <span>
                    {DAY_LABEL[slot.days] ?? slot.days} · {slot.start}–
                    {slot.end}
                  </span>
                </div>
              ))}
          ...
          {store.address_city && store.address_state ? (
            <div className="flex flex-col gap-1">
              ...
              {store.address_street && (
                <div className="pl-[23px] text-[13px] text-z-text-muted">
                  <span>Endereço:</span> {store.address_street}
                  {store.address_number && `, N° ${store.address_number}`}
