    ...
    (set, get) => ({
        ...
        set((state) => {
          ...
          if (removed) {
            track("remove_from_cart", {
              store_id: removed.product.store_id,
        ...
        set((state) => ({
          items: state.items.map((i) => {
            ...
            return {
              ...i,
              quantity:
                max <= 0 ? i.quantity : Math.min(Math.max(1, quantity), max),
            };
          }),
        })),
      ...
      applyCoupon: (coupon) => {
        track("coupon_applied", {
          ...
          discount_value: coupon.discountInCents,
        });
        set({ coupon });
      },
      clearCoupon: () => {
        const current = get().coupon;
        if (current) track("coupon_removed", { coupon_code: current.code });
        set({ coupon: null });
      },
      subtotalInCents: () =>
        get().items.reduce(
