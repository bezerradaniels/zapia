...
export function StatCard({ label, value, icon, color = "default" }: Props) {
  ...
  return (
    <div
      className={`flex items-center gap-4 rounded-2xl border border-gray-100 ${c.bg} px-5 py-4 shadow-sm`}
    >
      {icon && (
        <div
      ...
      <div className="min-w-0">
        ...
        <p className={`mt-0.5 text-2xl font-bold leading-none ${c.text}`}>
          {value}
        </p>
      </div>
    </div>
