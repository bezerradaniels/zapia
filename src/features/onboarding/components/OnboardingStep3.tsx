...
export function OnboardingStep3() {
  ...
  const {
    ...
    defaultValues: {
      ...
      delivery_hours: draft?.delivery_hours ?? [
        { days: "all", start: "08:00", end: "18:00" },
      ],
      delivery_area_scope: draft?.delivery_area_scope ?? "city_only",
      delivery_area_custom_locations:
        draft?.delivery_area_custom_locations ?? [],
    },
  });
  ...
  const deliveryAreaOptions = [
    {
      value: "city_only",
      label: step1Draft?.address_city
        ? `Apenas na cidade (${step1Draft.address_city})`
        : "Apenas na cidade",
    },
  ...
  const onSubmit = async (values: Step3Values) => {
    ...
    try {
      await patchStore(session.storeId, {
        ...
        delivery_area_custom_locations:
          values.delivery_area_custom_locations ?? [],
      });
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        ...
        {errors.accepted_payment_methods && (
          <p className="text-xs text-red-500">
            {errors.accepted_payment_methods.message}
          </p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {errors.accepted_shipping_methods && (
          <p className="text-xs text-red-500">
            {errors.accepted_shipping_methods.message}
          </p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {errors.delivery_area_scope && (
          <p className="text-xs text-red-500">
            {errors.delivery_area_scope.message}
          </p>
        )}
