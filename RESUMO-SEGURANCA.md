...
- `src/lib/sanitize/sanitizeHtml.ts` _(novo)_ — sanitização com DOMPurify (C-1/H-2)
...
- `supabase/config.toml` _(novo)_ — `verify_jwt` por função
...
_O que faz:_ liga as proteções de banco (pedidos, admin, upload, vendedores, colunas da loja).
